WITH f2020_f3 AS (
    SELECT * FROM {{ source('bronze', 'f2020_f3') }} 
)

, f2020_f3_renamed AS (

    SELECT
        CAST(2020 AS INT) AS "YEAR",
        -- =====================
        -- Identifiers
        -- =====================
        ...
        CAST("F3E074" AS NUMERIC) AS "F3E074",     -- Total expenses - Operations & maintenance
        CAST("F3E075" AS NUMERIC) AS "F3E075",     -- Total expenses - Depreciation
        CAST("F3E076" AS NUMERIC) AS "F3E076",     -- Total expenses - Interest
        CAST("F3E077" AS NUMERIC) AS "F3E077",     -- Total expenses - All other
        ...
        CAST("F3G01" AS NUMERIC) AS "F3G01",       -- Pretax income
        CAST("F3G02" AS NUMERIC) AS "F3G02",       -- Total revenues
        CAST("F3G03" AS NUMERIC) AS "F3G03",       -- Total equity
        CAST("F3G04" AS NUMERIC) AS "F3G04",       -- Total assets
        CAST("F3G05" AS NUMERIC) AS "F3G05",       -- Adjusted equity
        CAST("F3G06" AS NUMERIC) AS "F3G06",       -- Plant-related debt
        CAST("F3G07" AS NUMERIC) AS "F3G07"        -- Total expenses (alternate)
