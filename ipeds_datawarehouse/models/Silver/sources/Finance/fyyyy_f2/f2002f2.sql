WITH f2002_f2 AS (
    SELECT * FROM {{ source('bronze', 'f2002_f2') }} 
)
, f2002_f2_renamed AS (

    SELECT 
        CAST(2002 AS INT) AS "YEAR",
        -- =====================
        -- Identifiers
        -- =====================
        ...
        CAST(NULL AS NUMERIC) AS "F2A15",          -- Construction in Progress
        CAST(NULL AS NUMERIC) AS "F2A17",          -- Total Plant, Property, and Equipment
        CAST(NULL AS NUMERIC) AS "F2A18",          -- Accumulated depreciation
        CAST(NULL AS NUMERIC) AS "F2A19",          -- Property, Plant, and Equipment, net
        CAST(NULL AS NUMERIC) AS "F2A20",          -- Intangible Assets, net
        ...
        CAST(NULL AS NUMERIC) AS "F2D17",          -- Net assets released from restriction - Total
        CAST(NULL AS NUMERIC) AS "F2D18",          -- Net total revenues, after assets released from restriction - Total
        ...
        -- Endowment
        -- =====================
        CAST(NULL AS NUMERIC) AS "F2H01",         -- Endowment start
        CAST(NULL AS NUMERIC) AS "F2H02",         -- Endowment end
        CAST(NULL AS NUMERIC) AS "F2H03",         -- Change in endowment net assets
        CAST(NULL AS NUMERIC) AS "F2H03A",       -- New gifts and additions
        CAST(NULL AS NUMERIC) AS "F2H03B",       -- Endowment net investment return
        CAST(NULL AS NUMERIC) AS "F2H03C",       -- Spending distribution for current use
        CAST(NULL AS NUMERIC) AS "F2H03D"        -- Other changes in value of endowment net assets

    FROM f2002_f2

)

SELECT * FROM f2002_f2_renamed
