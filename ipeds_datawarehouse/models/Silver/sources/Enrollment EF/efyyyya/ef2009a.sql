WITH ef2009a AS (
    SELECT * FROM {{ source('bronze', 'ef2009a') }}
),
...
ef2009a_renamed AS (
    SELECT
        ...
        -- =====================
        COALESCE(NULLIF("EFAIANT", '')::INTEGER, NULLIF("EFRACE19", '')::INTEGER ) AS "EFAIANT",
        COALESCE(NULLIF("EFAIANM", '')::INTEGER, NULLIF("EFRACE20", '')::INTEGER) AS "EFAIANM",
        COALESCE(NULLIF("EFAIANW", '')::INTEGER, NULLIF("EFRACE21", '')::INTEGER) AS "EFAIANW",
        ...
        -- Hispanic / Latino
        -- =====================
        COALESCE(NULLIF("EFHISPT", '')::INTEGER, NULLIF("EFRACE21", '')::INTEGER ) AS "EFHISPT",
        COALESCE(NULLIF("EFHISPM", '')::INTEGER, NULLIF("EFRACE09", '')::INTEGER) AS "EFHISPM",
        COALESCE(NULLIF("EFHISPW", '')::INTEGER, NULLIF("EFRACE10", '')::INTEGER) AS "EFHISPW",
